...
if grep -q '^run_disk_command()' scripts/abora-installer.sh \
  ...
  && grep -q 'run_disk_command 120 mkfs.ext4 -F -L ABORA_ROOT' scripts/abora-installer.sh; then
  pass "runtime: installer bounds destructive disk commands and device settling"
else
  fail "runtime: installer bounds destructive disk commands and device settling"
fi

if grep -q '^check_existing_esp()' scripts/abora-installer.sh \
  ...
  && ! grep -q -- '--part 2' scripts/abora-installer.sh; then
  pass "runtime: installer validates reused ESPs and registers the selected EFI partition"
else
  fail "runtime: installer validates reused ESPs and registers the selected EFI partition"
fi

if grep -q 'target-flake.lock' nix/profiles/live.nix \
  && grep -q '/etc/abora/target-flake.lock' scripts/abora-installer.sh \
  && grep -q '/etc/abora/pkgs/abora-update-resolver-deps.json' scripts/abora-installer.sh \
  && grep -q '/etc/abora/pkgs/abora-plan-tool-deps.json' scripts/abora-installer.sh; then
  pass "runtime: installer preflight checks release lock and offline package assets"
else
...
if [[ -f tools/abora-installer/Cargo.toml ]] \
  ...
  && grep -q 'pkgs.abora-installer}/bin/abora-installer --legacy-script /etc/abora/installer.sh' nix/profiles/live.nix; then
  pass "runtime: Rust installer front controller is packaged and wired into the live ISO"
else
  fail "runtime: Rust installer front controller is packaged and wired into the live ISO"
fi
